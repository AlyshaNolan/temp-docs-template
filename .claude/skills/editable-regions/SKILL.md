...
Work inside a page-section wrapper that composes building blocks (model: `src/components/page-sections/collections/card-collection/CardCollection.astro`).
...
**A. Children rendered through `renderBlock`** (structured arrays: `buttonSections`, `contentSections`, accordion/card/step content). Only the wrapper needs the prop — `renderBlock` stamps `data-editable="array-item"` + `data-id={block._component}` on each row:

```astro
<ButtonGroup buttonSections={buttonSections} data-children-prop="buttonSections" />
```

**B. Fixed child component mapped manually** (a wrapper iterating its own `{Name}Item`). The wrapper owns the array container; you mark each child yourself:

```astro
<ol class="steps" data-children-prop="items">
  {
    items?.map((item) => (
      <StepsItem
