...
- The user pastes an image of a UI section (hero, feature grid, stats row, card grid, CTA) and wants a working component.
...
- **Layout** — single centered column, split (image + text), grid of N items, stacked blocks, or a heading area above a repeating group.
...
- **Interaction** — accordion, tabs, content selector, hover. Each maps to a CSS-first technique or an existing wrapper — see the CSS-first table in create-component's [component-templates.md](../create-component/component-templates.md#composition-and-css-first-rules). Compose the wrapper; do not reimplement its behavior.
...
Reference implementation for a heading-over-grid section: read `src/components/page-sections/collections/card-collection/CardCollection.astro` and its sibling YAML. It is the canonical shape — a `CustomSection` wrapping `SimpleText` / `Heading` / `Text` and a `CardGrid` that maps an array prop to its children. Mirror it rather than inventing a new structure.
