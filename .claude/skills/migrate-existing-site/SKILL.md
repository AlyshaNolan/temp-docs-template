...
- Only adding/editing pages in an already-migrated site — [docs-content-authoring](../docs-content-authoring/SKILL.md) or [page-content-authoring](../page-content-authoring/SKILL.md).
...
| Capture                                                     | Detail                                                                                                                                                                                                                                                                        |
| ----------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Pages                                                       | Every route/URL, and what it's for (overview, guides, reference, changelog...)                                                                                                                                                                                                |
...
| Content types                                               | How is the documentation grouped in its sidebar? Is there a changelog? Any other repeating content collection?                                                                                                                                                                |
...
Follow [docs-content-authoring](../docs-content-authoring/SKILL.md) to convert each page to `.mdx` in `src/content/docs/`, converting HTML body content to Markdown and downloading images per Assets below. Set `group` and `order` on every page as you go — that, not the folder, is what builds the sidebar, and a page without a `group` is invisible in it.

**Done-check:** every page renders, appears in the right sidebar group, and `npm run lint:links` reports no dead internal links.
...
Download every source image into `src/assets/images/` (Astro optimizes images from this path), organizing into subdirectories when the site has many (`src/assets/images/screenshots/`, `src/assets/images/diagrams/`). Use `/src/assets/images/placeholder.jpg` as a stand-in during phases 5–7 and swap in the real path once downloaded — write the real alt text immediately even against the placeholder, so nothing needs a second pass.
