...
| One field is free text while its siblings are configured                                            | Its `_inputs` entry is in a file the loading document doesn't read. Only the **first** `_inputs_from_glob` entry has been observed to reach the editor, so a second shared file is not applied — inline the field in the component's own `inputs.yml`.                              |
| An array renders as free text in the Content Editor but works in the page builder                   | The `_structures` it names is declared inside `structure-value.yml`, which `snippets.yml` cannot see. Move it to `.cloudcannon/structures/`.                                                                                                                                        |
