...
- An interactive component (the content selector, etc.) works on the live site but is dead in the editor.
...
| Browser console in the editor | open the CloudCannon Visual Editor, then browser dev tools | the same `Component not found` warning when the editor re-renders a block; `[editor-live-sync]` logs (DEV only); `ContentSelector: skipping setup` debug lines. |
...
| `npm run check` fails: `DRIFT .claude/skills`                         | [Skills drift](#skills-drift)                             |
