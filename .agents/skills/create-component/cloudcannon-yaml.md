...
**MUST NOT** hand-type the block — `npm run new:component` copies it in for you, slicing `hero-center.cloudcannon.inputs.yml` from the marker down. If you are writing a page section by hand, copy from that marker and edit only the component-specific fields above it.

Every page section follows this shape, and `lint:cms` + `lint:schema` both pass over it — so any page section is a valid model. `hero-center` remains the canonical donor because the scaffolder slices from it.
...
| `formBlocks`              | Form field areas (Form)             |
...
| Tier       | Contexts                                                                                            | Takes                                                                           |
| ---------- | --------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------- |
| **Wide**   | `containerSections`, `splitSections`, `contentSelectorSections`                                     | every wrapper and core element, **including its own type**                      |
| **Narrow** | `cardSections`, `gridItemSections`, `masonryItemSections`, `accordionSections`, `stepsItemSections` | content blocks only — card, button-group, accordion, steps, form, core elements |

A narrow context is a card-sized cell inside a measured layout, so it takes **no layout wrapper**: no split, grid, masonry, card-grid or content-selector.

**Why the tiers are uniform, including self-nesting:** the contexts form a graph — allowing `split` inside `containerSections` also makes everything in `splitSections` reachable from a custom section. Any exclusion that some other wrapper routes around is not a restriction, only an inconsistent picker: the editor is told "not here", then finds it works one wrapper down. `lint:nesting` computes that reachability and fails on an exclusion that buys nothing.
...
**To place a new wrapper:** add its `structure-value.yml` path to every context in the tier it belongs to (all three wide ones for a layout wrapper; all eight for a content block). Copy an existing entry for the exact path format (leading `/src/...`).

**To give a component a new nested content area:** create `.cloudcannon/structures/{name}Sections.cloudcannon.structures.yml` (copy `cardSections`), list the components allowed inside, and reference it from the array input as `_structures.{name}Sections`.

## snippets.yml (MDX-insertable page sections only)

Any component an author should be able to insert into a documentation page's body needs this. Model on `alert.cloudcannon.snippets.yml` for a core element, `faq-section.cloudcannon.snippets.yml` for a page section.
