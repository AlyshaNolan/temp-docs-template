...
- **The site chrome is editable on canvas.** The header, footer and breadcrumbs were data-panel-only because their content lives in `src/data/*.json` rather than on the page being edited. An editable region can bind across files — a `data-prop` beginning `@data[key]` resolves against that dataset, reading back and writing through — so they are now plain regions: the topbar's mark, wordmark, version and **top-level links** (an `array` region, so add, remove and reorder work on canvas), the footer's text, links and socials, the announcement bar's message, and the breadcrumb's Home crumb. The current crumb binds to the open page's `title`, so editing it renames the page. `src/data/breadcrumbs.json`, `src/data/footer.json` and `src/data/announcementBar.json` join `docsSite` under `data_config`, which is what makes the prefix resolve. `Footer.astro` and `AnnouncementBar.astro` take an `editableSource` prop because they are both site chrome and page-builder blocks — the shell passes `@data[footer]`, and placed in a page builder the bindings fall back to the page's own frontmatter.
- **The editor's switches take effect live.** `showCopyPage`, `showFeedback`, `showPager` and `showToc` on a page, `search`, `themeToggle`, `copyPage.enabled` and `feedback.enabled` in `docsSite.json`, and `enabled` in `announcementBar.json`, now show and hide their control in the Visual Editor without a rebuild. A boolean has no editable-region type, so this goes through the JavaScript API — but the reason it could not work before is that the controls were gated out of the markup (`{showCopyPage && <CopyPage/>}`), which leaves nothing to reveal when a switch is turned back on. Each is now always rendered and marked `data-toggle-hidden` when off. A control gated by both a page switch and a site switch is the AND of the two.
- **The derived navigation updates live in the Visual Editor.** Where a page sits in the sidebar and what its breadcrumb trail says are _derived_ by `getDocsNav()` from the whole `docs` collection joined with `docsSite.json` — no single path holds them, so no region can bind. `src/components/utils/siteChrome.ts` patches them through the CloudCannon JavaScript API instead: editing a page's **Group** moves its sidebar entry between groups (building a group the page is the first into, pruning the one it left) and carries the group crumb with it, **Order** repositions it among its siblings, and **Title** renames its sidebar link. From `docsSite.json`: the sidebar's lead link, and group order and collapsed state. Previously every one of these needed a rebuild to see.
- Renaming a `navGroups` entry deliberately renames nothing on canvas: membership comes from each page's `group` frontmatter, so the old name survives in the alphabetical tail, which is exactly what the rebuild produces.

- **An on-page theme selector, driven by the CloudCannon Visual Editor JavaScript API.** `src/data/theme.json` holds a light and dark accent colour, a light and dark brand colour, and a corner radius. `BaseLayout` turns them into custom properties on `<html>` and the theme files read each as `var(--accent-light, var(--teal-700))`, so a cleared field falls back to the shipped design instead of blanking a token. `src/utils/themeTokens.mjs` derives each colour's family — hover, surface, border, and the legible text colour on top, picked by relative luminance — so the editor chooses one swatch per role. In the Visual Editor a swatch button opens the file in a data panel and the page repaints as the colour picker is dragged: `file.data.edit()` to open CloudCannon's own inputs, `file.data.addEventListener("change", …)` to apply, and `style.setProperty()` on `<html>` to repaint. Custom properties inherit, so sections pinning their own `data-theme` follow in the same frame with no re-render. The button ships `hidden` and is revealed only by `editor-live-sync.js`, so a reader never sees it.
...
- Inline editing of a documentation page's **title** and **description** — both are editable regions on the page now, not data-panel-only fields. The `show*` switches stay in the data panel: an editable region can only bind text, an image or an array, never a boolean.
...
- The Steps section has a CloudCannon snippet, so an editor can insert a numbered walkthrough into a documentation page from the Content Editor's `+` menu the way FAQ Section already could.
- **The starter is a documentation site.** A grouped sidebar, an on-this-page rail with scroll tracking, breadcrumbs, prev/next paging, a mobile drawer, ⌘K search, dark mode, a print layout and a 404 that searches — all in `src/layouts/Docs.astro`, which every route goes through.
- A `docs` content collection: one `.mdx` file per page under `src/content/docs/`, path as URL. `group` and `order` frontmatter place the page in the sidebar; a page with no `group` still builds and is searchable, it just has no sidebar entry. Ordering of the groups themselves is `navGroups` in `src/data/docsSite.json`, so an editor changes it in CloudCannon rather than in a pull request.
- `src/data/docsSite.json` — the header (brand, version badge, search, theme toggle, top-level links), the sidebar group order, the "Edit this page" link, and the feedback block, in one CloudCannon-editable file.
- A `changelog` content collection: one file per release under `src/content/changelog/`, holding a version, a date, an optional free-text `summary` and its tagged lines. The summary renders between the version heading and the list, for what a release is about or an upgrade note. The **Changelog** block renders the whole collection newest first — `<Changelog />` is the body of `/changelog/` — so shipping a release is adding a file, and an editor adds one from a CloudCannon collection rather than by hand-editing a page. `limit` renders only the most recent few, for a release-notes teaser elsewhere. The collection has no URL of its own.
- Documentation building blocks, each with a CloudCannon snippet so an editor can insert it into a page body: **Code Block** (filename bar, copy button, line highlighting), **Code Tabs** (one sample in several languages, the reader's choice remembered site-wide), **Annotated Code** (numbered notes under the block rather than comments inside it), **Parameter List** (API reference entries), **Diagram** (Mermaid), **Figure** (caption, dark-mode variant, click-to-zoom) and **Changelog**.
- Markdown fences take `title="path"` for the header label and `{2,5-7}` for line highlights, and get the same panel and copy button as the component — a fence and `CodeBlock` are one object on the page.
- Search results list the headings that matched under the page they belong to, so a result opens the section a reader wanted instead of the top of a long page. The trigger is a labelled ⌘K control rather than an icon.
- "Was this page helpful?" at the foot of every documentation page. It is a real `<form method="post">` that works with JavaScript off; a script upgrades it to submit in the background. The default action posts to the site itself, which CloudCannon Forms captures with no backend — `/feedback-and-forms/` documents pointing it at Formspree, Netlify or your own endpoint.
- A print layout (`src/styles/base/_print.css`): chrome dropped, plain black on white, external URLs printed after their link text, headings never ending a page, code and tables never splitting mid-row, and disclosures printed expanded.
- Clicking a prose heading copies a link to that section, with a `#` affordance on hover.
- A **Copy page** menu beside every documentation page's title: the page as Markdown, the same Markdown wrapped as an LLM prompt, the page's link, the raw `.md` file, or the print dialog — the first thing on a documentation page to reveal that the site has a tuned print layout at all. Every page now has a Markdown twin at the same path with a `.md` extension (`/installation.md`), served by `src/pages/[...slug].md.ts` — the page's own MDX source, so nothing a component renders can go missing from the copy. Switched on site-wide by `copyPage` in `src/data/docsSite.json` and per page by `showCopyPage` frontmatter; `/copy-page/` documents it.
- `npm run lint:links` — every internal link in the built site must resolve to a page or an asset. A renamed page leaves dead links behind and nothing else notices.
- `alignmentHorizontal` on Hero Center, Stats and Card Collection, so a documentation landing page can left-align its headings instead of centring them.
- A `layout` option on Definition List: `stacked` reads as a glossary (term, then its definition indented beneath), `grid` keeps the previous two-column spec-row shape.
- A `tip` variant on Callout, alongside note, warning, danger and info.
...
- The sidebar carries **two levels**. A page nests under the page at its parent path — `theming/token-reference.mdx` under `theming.mdx` — and inherits its group, so the levels can't disagree. Breadcrumbs and the prev/next pager follow, depth-first. Nesting is the file tree, so moving a page is the only thing that changes it. `/theming/token-reference/` ships as the worked example.
- **Print Button** (`building-blocks/core-elements/print-button`): opens the reader's print dialog. It ships hidden and reveals itself once its script runs — printing needs JavaScript, and every browser's own shortcut still works, so a dead control would be worse than none. Wired into the Print & PDF page.
- **Code Diff** (`building-blocks/core-elements/code-diff`): a code sample whose changed lines are marked by a leading `+`, `-` or `~`. The marker is stripped before the grammar runs, so the sample keeps its own language colours instead of going flat the way a `diff` fence does, and the `+`/`-` signs are drawn in a gutter rather than being text — the copy button hands back clean source. Offered as a page-builder block and as an MDX snippet.
...
- `--color-bg-accent`, `--color-focus-ring` and the `--color-info-*` tokens now derive from `--color-accent*` instead of naming `--teal-*` directly. They were the three places a hardcoded hue leaked past the accent block, which meant a re-skin left the focus ring and info callouts teal.
- `--radius-*` is now one scale computed from `--radius-base` rather than nine independent values. The shipped base of `10px` reproduces the previous scale exactly.
- A custom accent or brand colour derives its hover, surface and border by blending, where the shipped teal used hand-picked ramp steps. Leaving `theme.json` on its defaults therefore shifts those three tokens by a few RGB points — `#115e59` to `#0e625c` for the light accent hover, and similarly for the surface and border tints.
...
- CloudCannon parses content as GitHub Flavored Markdown (`markdown:` in `cloudcannon.config.yml`). Its editor defaulted to plain CommonMark while Astro renders GFM, so a Markdown table came back out of the Content Editor as HTML — or as literal pipe characters — with nothing reporting an error. Strikethrough, autolinks and snippet-safe code fences come with it.
- A page section placed in a documentation page's prose no longer adds its own gutters, max-width or band padding — the article already supplies them, which is why every such usage had to switch all three off by hand.
- The Steps snippet's Content Editor fields are down to the five that matter (heading, subtext, the steps, orientation, image ratio) from twelve. The section-wrapper knobs it dropped were only ever set to "none" in prose, which the rule above now does.

- The overview page describes what the template actually is. It is a GitHub template copied with **Use this template** or deployed from CloudCannon, not an npm package: the hero's `npm create docsmith@latest` command line is gone, `/installation/` documents both routes and says that CloudCannon needs a paid plan, and the closing call to action points at the docs rather than an installer. The statistics are measured rather than invented — the old row claimed zero runtime dependencies for a site with sixteen, and a search index and build time for two hundred pages that do not exist.
- "Instant search" is now "built-in search" throughout. The old phrasing reads as a hosted search service; the index is built by Pagefind during the build and shipped with the site, with nothing to sign up for.
- The documentation's configuration samples are the real `src/data/docsSite.json` rather than an invented `docsmith.config.js` with an `algolia`/`local` search provider — the file did not exist and the option never has.

- The design is retuned end to end: a blue-cast neutral ramp with a teal accent (`--teal-700` on light, `--teal-300` on dark), Instrument Sans and JetBrains Mono, a 10px radius, a documentation type scale (42 / 24 / 18) and a 1.72 body measure. All of it is tokens — `src/styles/variables/` and `src/styles/themes/` — so it remains one file's worth of work to change again.
- `--color-accent` is a tier of its own, separate from `--color-brand`. Links, the active sidebar item, focus rings and code annotations use the accent; primary buttons stay ink on paper. Changing the accent no longer turns the page into a wall of one hue.
- Callout is the documentation callout from the design: a coloured left bar, a bold first line, and no icon unless you ask for one. `success` is gone; `tip` replaces it.
- Documentation prose has its own stylesheet (`src/styles/base/_prose.css`) covering everything Markdown can produce. It is bottom-margins only, so adjacent margins collapse and a section break is one gap rather than the sum of two.
- Code blocks stay dark in both themes. A sample that inverts with the page re-reads as a different snippet, and syntax colours are tuned for one ground.
- One route serves both content collections. `src/pages/[...slug].astro` mounts `pages` and `docs` at the site root, and a slug claimed by both throws at build time naming the two files rather than resolving by accident.
- The site header and sidebar are purpose-built for documentation (`docs-topbar`, `docs-sidebar`) and the mobile drawer is CSS-only — a hidden checkbox and a `<label for>`, coupled by an id both components import — so it works in the CloudCannon editor, which doesn't run inline scripts.
- The footer is one line: legal text, links, socials.
- Smoke tests drive page-builder-only components through the `/preview-renders/<component key>/` routes, added by `COMPONENT_PREVIEWS=true`, instead of the component-library UI. Seventeen tests now cover the drawer, theme toggle, search and its sub-results, code tabs and copy, diagram rendering, the helpful vote and the on-this-page spy.
- The agent-facing component catalog is generated from each component's own YAML by `npm run docs:catalog`; `docs:catalog:check` fails on drift. A hand-maintained list rots the moment a prop is renamed.
- `npm run reset:starter` clears the documentation demo and the template's branding, and writes a blank Introduction page rather than leaving an empty sidebar.
...
- The overview page is rebuilt to the design: a pill badge and an accent word in the hero, the install command beside the primary button, four stat tiles, a tabbed panel demoing callouts, code tabs, annotations, diagrams and reference lists with the real components, a grid of links into the documentation, and an inverse closing band.

- The announcement bar and the overview's closing band drop from a full brand/inverse fill to the accent tint, with an accent-coloured link carrying the emphasis. Both were louder than anything they sat next to. `backgroundColor: inverse` is still there for a band that should shout.
- A secondary button borrows an outline on a tinted band (`accent`, `highlight`), where its neutral surface fill all but vanished.
- Every section on the overview page contributes the same vertical padding, so every gap between them is one even 48px rather than a mix of 48 and 56.
- A linked card lifts, brightens its border and slides a trailing heading icon into the accent on hover, instead of scaling up — a scale blurs text mid-transition and reads as a photo zoom, which is wrong for a card that is mostly words. Under `prefers-reduced-motion` the border and accent still change; only the movement goes.
- The sidebar group on a documentation page is a picker rather than a free-text field, listing the groups from `navGroups` in `src/data/docsSite.json`. A typo no longer strands a page in a group of its own, and the input links straight to the file when a new group is needed.

### Removed

- The `updated` frontmatter key on documentation pages, and `editPageBaseUrl` from `src/data/docsSite.json`. Both are derived now — see Changed above.

- Everything left in the library that a documentation site has no use for, after checking each candidate against the content, the docs pages, and every other component: the **Hero Split**, **Page Header**, **CTA Center**, **Feature Grid** and **Feature Split** page sections; the **Carousel**, **Image Carousel**, **Video Modal**, **Bento Box**, **Timeline** and **Stack** wrappers; the standalone **Modal** block; the **Table**, **List** and **Divider** core elements (Markdown renders all three in a docs page); and the unused `Avatar` util. Their CloudCannon YAML, preview recipes and SVGs, picker entries and smoke tests went with them, along with the now-orphaned `carouselSections`, `bentoBoxSections`, `modalSections` and `stackSections` picker contexts and the three `embla-carousel` dependencies. `ModalShell` stays — it is what the search dialog is built from.
- The blog: posts, routes, tag archives, pagination, RSS and the Latest Posts section. A documentation site's release notes are the changelog page.
- The `/component-docs/` component-library UI, its gallery and its drag-and-drop builder. A documentation-site template documenting itself through a second, separate documentation system was confusing to operate; the component reference is now the generated catalog under `.agents/skills/`, and `npm run previews:montage` renders every component thumbnail as one contact sheet. `npm run build:with-library` and `DISABLE_COMPONENT_LIBRARY` are gone with it — `npm run build` is the only build.
- Marketing-only page sections: Pricing Tiers, Pricing Comparison, Testimonial Section, Testimonial Wall, Team Grid, Logo Cloud, Gallery Grid, Contact Split, CTA Form, Feature Slider and the Timeline section, plus the Testimonial, Rating and Pagination core elements. Pagination hardcoded `/blog` URLs and had nothing left to paginate.
- The mega-menu navigation cluster — Main Nav, Bar, Mobile and Side — replaced by the documentation topbar and sidebar. `src/data/mainNav.json` is replaced by `src/data/docsSite.json`.
- The `blog-mdx-content` skill, replaced by `docs-content-authoring`.

### Fixed

- Sidebar and breadcrumb edits in the Visual Editor survive navigating to another page. The live patcher read only `currentFile()`, so moving a page to another group showed correctly until you opened a different page — which is served exactly as it was built, with the unsaved edit nowhere in it. It now re-derives from the whole collection, which includes edits that have not been built yet.
- The `showToc`, `showFeedback`, `showCopyPage` and `showPager` switches no longer hide their control on load. `file.data.get()` returns raw frontmatter rather than the Zod-parsed entry the build sees, and only one shipped page writes these keys out — so an absent key read as `undefined` and was treated as off, hiding the copy button, the feedback block, the pager and the table of contents on nearly every page. Absent now reads as `true`, matching the content schema. Controls gated by both a page and a site switch also wait for both handles: an unread handle and a switched-off control were indistinguishable.
- The announcement bar no longer renders CloudCannon's `_schema` key as an HTML attribute on its `<aside>`. `Docs.astro` spreads the whole data file into it, and only `_component` was being destructured out.
...
- A Markdown fence's `title=` never reached the page. `_prose.css` labels a fence's header from `data-title` and falls back to the language, and nothing set the attribute — so every fence on the site was headed "bash" or "json" instead of the filename the author wrote, while the `CodeBlock` component beside it showed one. The fence-meta transformer now lifts it, next to the line-range parsing it already owns.
- Code samples overflowed the right edge of a Content Selector in its panel variant. The panel's tab strip scrolls horizontally, so the open panel escapes a zero-width grid item and sizes itself to `100cqi`; the panel's own padding then shifted it right without narrowing it, pushing exactly one padding's worth of every block past the border — invisible in prose, obvious in a code block with its own scroller. The padding moved to the escaping element and `100cqi` now measures the tab strip's content box rather than the section around it.
- Changing a Content Selector's tab position or panel variant in the Visual Editor did nothing until a reload. Both classes sat on the element carrying the array editable region, and CloudCannon's re-render keeps a region's root and swaps only its contents. They moved to a wrapper inside the component root, which is re-rendered like any other content.
- The Hero Center eyebrow pill could not be edited on the canvas: it carried `data-prop` with no `data-editable`, so CloudCannon had a field name and no region to attach it to.
- `headingHighlight` on Hero Center lost its accent colour in the Visual Editor. A text editable region owns its whole subtree and flattens it to plain text, taking the highlight's `<span>` with it; the heading now drops that region when the highlight matches, and the component re-render — which knows how to split the heading — keeps ownership of the markup.

- A component rendering sibling `<li>`s inside a documentation page came out misaligned: the prose stylesheet's `li + li` spacing is a descendant rule, so it reached past the article's own lists and put a 4px top margin on every item of a component's list but the first — enough to knock the Steps section's columns off their baseline, with nothing in the component to explain it. That rule is now scoped to the article's own lists, as the padding rule beside it already was.
- The documentation sidebar hid its top links once you reached the bottom of a page — on a laptop the first link sat behind the topbar until you scrolled the article back up, and no amount of scrolling the nav itself brought it back. Both sticky rails are pinned inside their grid item, which ends where the article column does, so the footer below it pushed them up and under the topbar; the sidebar also stretched to the full viewport height however few links it held, so the push ate links rather than empty space. Each rail now sizes to the gap it actually has — `100vh` less the topbar and the footer — claims the footer's top margin to stay pinned across it, and scrolls internally when the nav is taller than that gap. A smoke test at a laptop viewport asserts the first link clears the topbar at the foot of a page.
- Multi-line code written inline in an MDX prop lost one level of indentation. MDX strips up to two leading spaces from every continuation line of a JSX attribute, so the Annotated Code sample on `/writing-content/` rendered with its nested lines flush left — the page built and nothing warned. The documentation samples are hoisted into an ESM block, which MDX leaves alone, and the docs-content-authoring skill now says to write them that way.
- `npm run lint:links` passed over a dead link when `dist/` was older than the content that broke it. It only built the site when `dist/` was missing, so a stale build was checked as-is and the dead href simply wasn't in the HTML it read — a false pass on exactly the rename the check exists to catch. It now rebuilds when any of `src/`, `public/`, `astro.config.mjs` or `site-fonts.mjs` is newer than the build.
- The homepage's "Get started" button linked to `/installation-overview/`, which is not a route. It points at `/installation/`.
- A Markdown fence's header bar sat 16px below the panel's rounded top edge. The block padding was on the `<pre>`, and a fence's header is a `::before` _inside_ it — so the padding pushed the header down. The padding is on the `<code>` now, which is also where the component path wants it.
- Two consecutive Markdown fences touched. The shared code-surface rule set `margin: 0` on them from `@layer components`, which beat the prose margins in `@layer base`. That reset is now scoped to the `<pre>` inside a `CodeBlockSurface`, where a UA margin genuinely has to go.
- Blocks in prose — callouts, code, diagrams, parameter lists, tables, figures — sit an even 24px from whatever is either side of them, instead of 44px below. A bottom-margin-only model can't tell a sibling of the same kind from the next section, so four stacked callouts were 44px apart; the section break is the heading's own top margin anyway. The prose rhythm is now 8px under a heading, 24px around a block, 44px before the next heading.
- `Heading` renders nothing when it has nothing to render. `Astro.slots.has()` is true for a slot holding an empty string — which is what a page section passes for an optional heading left blank — so an empty `<h2>` shipped: a hole in the document outline, and a flow sibling that stopped the next block being a first child and handed it a 48px top margin it should not have had. That was the uneven spacing on the overview page: gaps alternated 96px and 48px depending on whether the section's heading was set.
- The hero's lede sat 8px under its display heading (body text's `tight` type default), which read as a collision against a -0.045em h1. Both it and the eyebrow pill now use 24px, set through `--space-before`.
- The hero's eyebrow pill sat 48px from the heading — the flow system's `loose` default for a heading. It is 24px now, set through `--space-before` rather than a raw margin, which the utils layer would have overridden (`npm run test:flow-margins` catches exactly that).
- A `Heading` with a `size` no longer squashes its words together. The size class sits on an inner element while `letter-spacing` was set on the `<h*>` in `em` of its base size — which inherits as an absolute pixel value, so an 18px heading carried a 52px heading's tracking. Each size now states its own.
- A CTA Split with no image lays its text and buttons out across the section instead of stacking them in a 400px column with the rest of the section empty — which is what its own label ("text on one side and buttons on the other") always promised.
- Escape closes the search overlay on the first press when a query has been typed. A non-empty `<input type="search">` consumes Escape as a native field reset, which spent the popover's close request — the overlay needed two presses.
- Content Selector tabs work when two items share a heading. Item ids were derived from the title, so duplicate headings produced duplicate ids and every `<label for>` pointed at the first radio: clicking any tab opened panel one.
