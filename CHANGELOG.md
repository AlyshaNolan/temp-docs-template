...
- `src/utils/navOrder.ts` holds the two rules that order the sidebar — page sort and group order — so the build-time derivation and the editor's live patcher cannot drift apart and land a page in two different places. Renaming a `navGroups` entry deliberately renames nothing on canvas: membership comes from each page's `group` frontmatter, so the old name survives in the alphabetical tail, which is exactly what the rebuild produces.
