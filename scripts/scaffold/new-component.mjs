...
const SECTION_INPUTS_DONOR = join(
  componentsDir,
  "page-sections/heroes/hero-center/hero-center.cloudcannon.inputs.yml"
);
...
Next steps:
  ...
  3. Usable inside a documentation page? Add ${slug}.cloudcannon.snippets.yml
     so it appears in the Content Editor's insert menu (see the
     create-component skill for the template).
  4. Add it to a page under src/content/ so it ships composed, not just in the
     picker — and so the smoke tests can reach it.
  5. npm run check
